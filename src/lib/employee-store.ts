...
export async function gateUnlock(password: string, remember: boolean): Promise<boolean> {
  const { verifyEmployeeGate } = await import("@/lib/employee-claim.functions");
  const { ok } = await verifyEmployeeGate({ data: { gatePassword: password } });
  if (!ok) return false;
...
export async function employeeSignUp(name: string, email: string, password: string, gatePassword: string): Promise<{ error?: string }> {
  if (!gatePassword) return { error: "Team passcode required." };
