  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        upstream.append("model", process.env.AI_STT_MODEL ?? "gpt-4o-transcribe");
        upstream.append("file", file, file.name || "recording.wav");
        if (typeof language === "string" && /^[a-z]{2}$/.test(language)) {
          upstream.append("language", language);
        }

        const res = await fetch(`${process.env.AI_BASE_URL ?? "https://api.openai.com/v1"}/audio/transcriptions`, {
          method: "POST",
