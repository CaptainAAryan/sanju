...
function MobilePage() {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!country.mobileLengths.includes(mobile.length)) {
      const allowed = country.mobileLengths.join(" or ");
      return setError(`Please enter a ${allowed}-digit ${country.name} mobile number.`);
    }
    if (!country.mobileRegex.test(mobile)) {
    ...
    <PageShell>
      <div className="mx-auto max-w-md px-5 py-10">
        ...
        <p className="mt-2 text-center text-muted-foreground text-sm">
          {country.flag} {country.name} · {country.dialPrefix} · {country.mobileLengths.join("/")} digits
        </p>
        ...
        <form onSubmit={submit} className="mt-8 space-y-4">
          <div className="relative">
            ...
            <input
              ...
              inputMode="numeric"
              value={mobile}
              onChange={(e) => { setMobile(e.target.value.replace(/\D/g, "").slice(0, maxLen)); setError(""); }}
